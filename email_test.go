...
func TestBuildMessage(t *testing.T) {
	...
	t.Run("plain text only", func(t *testing.T) {
		...
		if !strings.Contains(msgStr, "Content-Transfer-Encoding: quoted-printable") {
			t.Error("message should contain Content-Transfer-Encoding")
		}
		if !strings.Contains(msgStr, "MIME-Version: 1.0") {
			t.Error("message should contain MIME-Version")
		}
		if !strings.Contains(msgStr, "Message-ID:") {
			t.Error("message should contain auto-generated Message-ID")
		}
		if !strings.Contains(msgStr, "Date:") {
	...
	t.Run("multipart alternative", func(t *testing.T) {
		...
		if !strings.Contains(msgStr, "multipart/alternative") {
			t.Error("message should contain multipart/alternative")
		}
	})
	...
	t.Run("with attachments", func(t *testing.T) {
		...
		if !strings.Contains(msgStr, "Content-Disposition: attachment; filename=\"test.txt\"") {
			t.Error("message should contain attachment disposition")
		}
		if !strings.Contains(msgStr, "Content-Transfer-Encoding: base64") {
			t.Error("attachment should use base64 encoding")
		}
	})
	...
	t.Run("with Cc and ReplyTo", func(t *testing.T) {
		...
		if !strings.Contains(msgStr, "Reply-To: replyto@example.com") {
	...
	t.Run("with custom headers", func(t *testing.T) {
		...
		msgStr := string(msg)
		if !strings.Contains(msgStr, "X-Custom: value1") {
			t.Error("message should contain custom header")
		}
		// When Message-ID is set in headers, it should not be auto-generated
	...
	t.Run("non-ASCII subject", func(t *testing.T) {
		...
		msgStr := string(msg)
		if !strings.Contains(msgStr, "=?UTF-8?") {
			t.Error("non-ASCII subject should be RFC 2047 encoded")
		}
	})
}
